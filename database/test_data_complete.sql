...
SELECT id, name, status, occupied_at FROM restaurant_tables WHERE id IN (1, 2, 3, 4, 5);

SELECT 'ÓRDENES CREADAS:' AS info;
SELECT id, table_id, table_name, status, total_amount FROM restaurant_orders WHERE table_id IN (1, 2, 3, 4, 5);

SELECT 'ITEMS DE LAS ÓRDENES:' AS info;
SELECT oi.id, oi.order_id, oi.product_name, oi.quantity, oi.unit_price, oi.subtotal 
FROM order_items oi 
WHERE oi.order_id IN (SELECT id FROM restaurant_orders WHERE table_id IN (1, 2, 3, 4, 5));
